...
def pdb_to_npy_interpolate(seqlen, mask, pos_mask, mass=0.0785398163397448, c=(3.8)**2/2.3211):
    V = dct(np.eye(seqlen), norm="ortho")
    k = (np.pi/seqlen) * np.arange(seqlen); k[0]=k[1]
    Gk = c / (k**2 + mass**2)
    Gm1 = V@np.diag(1/Gk)@V.T

    G11 = Gm1[~mask][:, ~mask]
    G12 = Gm1[~mask][:, mask]
    mu = - np.linalg.inv(G11) @ G12 @ pos_mask
    val, vec = np.linalg.eigh(G11)
