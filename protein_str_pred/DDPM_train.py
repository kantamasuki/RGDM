...
from utils.DDPM_parsing import parse_train_args
...
def main():
    ...
    if not args.dry_run:
        model_dir = os.path.join(args.workdir, 'DDPM_'+str(args.time))
    else:
        model_dir = os.path.join(args.workdir, 'DDPM_dry_run')
    ...
    # data loader, optimizer など
    train_loader = DDPM_get_loader(args, splits, mode='train', shuffle=True)
    val_loader = DDPM_get_loader(args, splits, mode='val', shuffle=False)
